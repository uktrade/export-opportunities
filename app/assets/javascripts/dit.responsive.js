...
dit.responsive = (new function () {
 ...
 /* Create in-page <style> tag containing set media query breakpoints 
  * passed to dit.responsive.init()
  * @queries (Object) Media queries and label - e.g. { desktop: "min-width: 1200px" }
  *
  * Note: IE8 cannot handle media queries so this won't work for that browser but
  * we are using IE8 friendly way of appending the <style> tag. 
  **/
  function addResponsiveSizes(queries) {
    ...
    $(document.head).append('<style id="dit-responsive-css" type="text/css">' + css + '</style>');
  }

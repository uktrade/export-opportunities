...
ukti.EnquiryResponse = (function ($) {
  ...
  var returnResponseType = function () {
    return ukti.Utilities.getValueFromRadioButton(
      'enquiry_response[response_type]'
    );
  };
  ...
  var initTextEditor = function () {
    CKEDITOR.replace('editor1', {
      customConfig: ukti.config.ckeditorConfigPath,
    });
  ...
  var initToggleFieldEdit = function () {
    var els = document.querySelectorAll('.js-toggle-field-edit'),
      i;
  ...
  var initUploadWidget = function () {
    var els = document.querySelectorAll('.uploadWidget'),
      i;
    for (i = 0; i < els.length; ++i) {
  ...
  var formSubmitHandler = function (event) {
    ...
    if (isSignatureRequired() && isSignatureInvalid()) {
      ukti.Forms.addErrorToField(
        baseEl.querySelector('#enquiry_response_signature'),
        config.errorMessages.signature
      );
    ...
    if (isCommentRequired() && isCommentInvalid()) {
      ukti.Forms.addErrorToField(
        baseEl.querySelector('#enquiry_response_email_body'),
        config.errorMessages.comment
      );
  ...
  var isCommentRequired = function () {
    var response_type = returnResponseType();
    return (
      response_type === '1' || response_type === '2' || response_type === '3'
    );
  };

class OpportunityPresenter < BasePresenter
  ...
  def title_with_country
    if source('post') && ( opportunity.created_at < Date.parse(Figaro.env.POST_FORM_DEPLOYMENT_DATE) || ( opportunity.service_provider.country.region && opportunity.service_provider.country.region.name == 'no region' ))
      opportunity.title
    else

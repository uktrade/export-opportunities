...
module Hawk
  ...
  def authenticate(authorization_header, options)
    big_authenticate(authorization_header, { :server_response => true, :type => 'response' }.merge(options))
  end
  ...
  def calculate_time_offset(authorization_header, options)
    parts = parse(authorization_header)
  ...
  def big_authenticate(header, options)
    options = options.dup

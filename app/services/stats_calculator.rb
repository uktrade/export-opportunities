class StatsCalculator
  ...
  def results_with_filters(service_providers, date_range)
    ...
    enquiry_response = Enquiry.joins(:enquiry_response).sent.joins(:opportunity).where(opportunities: { service_provider_id: service_providers.map(&:id) }, enquiry_response: { created_at: date_range }).select('count(*)')
    [opportunities_submitted, opportunities_published, enquiries, enquiry_response]
  end

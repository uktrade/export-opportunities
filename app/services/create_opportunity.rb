class CreateOpportunity
  ...
  def call(params)
    Rails.logger.debug("VOLUMEOPS - Creating Opportunity...")
    ...
    if params[:slug].nil?
      Rails.logger.debug("VOLUMEOPS - Creating Opportunity - slugging...")
      opportunity.slug = CreateOpportunitySlug.call(opportunity)
      Rails.logger.debug("VOLUMEOPS - Creating Opportunity - slugging... done")
    end
    ...
    begin
      Rails.logger.debug("VOLUMEOPS - Creating Opportunity - saving...")
      opportunity.save!
      Rails.logger.debug("VOLUMEOPS - Creating Opportunity - saving... done")

      Rails.logger.debug("VOLUMEOPS - Creating Opportunity - fetching CPVs...")
      opportunity_cpvs&.each do |opportunity_cpv|
        Rails.logger.debug("VOLUMEOPS - Creating Opportunity - fetching individual CPV...")
        ...
        Rails.logger.debug("VOLUMEOPS - Creating Opportunity - fetching individual CPV... done")
        Rails.logger.debug("VOLUMEOPS - Creating Opportunity - adding sector from CPV...")
        add_sector_from_cpv(opportunity, cpv_id)
        Rails.logger.debug("VOLUMEOPS - Creating Opportunity - adding sector from CPV... done")
      end
      Rails.logger.debug("VOLUMEOPS - Creating Opportunity - fetching CPVs... done")

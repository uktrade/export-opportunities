class Admin::OpportunitiesController < Admin::BaseController
  ...
  def opportunity_params(contacts_attributes:)
    params.require(:opportunity).permit(:title, :slug, { country_ids: [] }, { sector_ids: [] }, { type_ids: [] }, { value_ids: [] }, :teaser, :response_due_on, :description, { contacts_attributes: contacts_attributes }, :service_provider_id, :ragg, :buyer_name, :buyer_address, :language, :tender_value, :source, :tender_content, :tender_url, :tender_source, :ocid, :industry_id, :industry_scheme)
  end

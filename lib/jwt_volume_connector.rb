class JwtVolumeConnector
  def call(username, password, hostname, url)
    Rails.logger.debug("VOLUMEOPS - Getting JWT...")
  ...
  def token(username, password, hostname, url)
    Rails.logger.debug("VOLUMEOPS - Getting JWT token...")
    unless username && password && hostname && url
      Rails.logger.debug("VOLUMEOPS - Getting JWT token... invalid inputs")
    ...
    Rails.logger.debug("VOLUMEOPS - Getting JWT token... done")
    post
  end
  ...
  def data(token, hostname, url, from_date, to_date)
    unless token && hostname && url
      raise Exception, 'invalid input'
      Rails.logger.debug("VOLUMEOPS - Getting JWT data... invalid inputs")
    end
    ...
    Rails.logger.debug("VOLUMEOPS - Getting JWT... done")
    ...
    Rails.logger.debug("VOLUMEOPS - Getting JWT... failed")

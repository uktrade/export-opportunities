...
RSpec.describe StatsSearchForm do
  describe '#service_providers' do
    it 'returns service providers ordered by name, preceded by the special "AllServiceProviders" option' do
      ...
      expect(form.service_providers).to eq [StatsSearchForm::AllServiceProviders, aachen, london, xanadu]
    end
  end

  describe '#service_provider_id' do
    it 'returns the id which was passed in' do
      expect(StatsSearchForm.new(service_provider: 17).service_provider_id).to eq 17
    end
  end
  ...
  describe '#valid?' do
    it 'is valid when it has a service provider and from- and to- dates' do
      ...
      form = StatsSearchForm.new(service_provider: 1, stats_from: from_date, stats_to: to_date)
    ...
    it 'is valid when the from_date is before the to_date' do
      ...
      form = StatsSearchForm.new(service_provider: 1, stats_from: earlier_date, stats_to: later_date)
      expect(form.valid?).to eq true
    end

    it 'is valid when the from_date is the same as the to_date' do
      date = { year: '2017', month: '1', day: '1' }
      form = StatsSearchForm.new(service_provider: 1, stats_from: date, stats_to: date)
      expect(form.valid?).to eq true
    end
    ...
    it 'is invalid when the from_date comes after the to_date' do
      earlier_date = { year: '2016', month: '1', day: '1' }
      later_date = { year: '2017', month: '1', day: '1' }
      form = StatsSearchForm.new(service_provider: 1, stats_from: later_date, stats_to: earlier_date)
      expect(form.valid?).to eq false
    ...
    it 'is invalid when the to_date is invalid' do
      ...
      form = StatsSearchForm.new(service_provider: 1, stats_from: valid_date, stats_to: invalid_date)
      expect(form.valid?).to eq false
    ...
    it 'is invalid when the from_date is invalid' do
      valid_date = { year: '2017', month: '2', day: '1' }
      invalid_date = { year: '2017', month: '2', day: '30' }
      form = StatsSearchForm.new(service_provider: 1, stats_from: invalid_date, stats_to: valid_date)
      expect(form.valid?).to eq false
    ...
    it 'is invalid when the from- or to-date is before SITE_LAUNCH_YEAR or after today' do
      ...
      form = StatsSearchForm.new(service_provider: 1, stats_from: invalid_date_too_early, stats_to: invalid_date_too_late)
      expect(form.valid?).to eq false
      expect(form.error_messages).to include(t('admin.stats.errors.invalid_date', field: 'From'))
      expect(form.error_messages).to include(t('admin.stats.errors.invalid_date', field: 'To'))
    end

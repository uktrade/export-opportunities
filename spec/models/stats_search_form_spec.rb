...
RSpec.describe StatsSearchForm do
  ...
  describe '#service_provider_id' do
    it 'returns the id which was passed in' do
      params = { granularity: 'Country', Country: { country_ids: ['120', '2', ''] }, ServiceProvider: { service_provider_ids: ['17'] } }
  ...
  describe '#valid?' do
    ...
    it 'is valid when the from_date is before the to_date' do
      ...
      params = { granularity: 'Country', Country: { country_ids: ['120', '2'] }, ServiceProvider: { service_provider_ids: ['17'] }, stats_from: earlier_date, stats_to: later_date }
      form = StatsSearchForm.new(params)
      expect(form.valid?).to eq true
    end
    ...
    it 'is valid when the from_date is the same as the to_date' do
      ...
      params = { granularity: 'Country', Country: { country_ids: ['120', '2'] }, ServiceProvider: { service_provider_ids: ['17'] }, stats_from: date, stats_to: date }
      form = StatsSearchForm.new(params)
      expect(form.valid?).to eq true
    end

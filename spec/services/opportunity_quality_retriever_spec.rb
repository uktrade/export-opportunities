...
describe OpportunityQualityRetriever, type: :service do
  describe '#call' do
    it 'returns valid when no errors found' do
      ...
      allow_any_instance_of(OpportunityQualityRetriever).to receive(:quality_check).with(Figaro.env.TG_HOSTNAME!, Figaro.env.TG_API_KEY!, opportunity.title).and_return(no_errors_result_json)

      response = OpportunityQualityRetriever.new.call(opportunity)

      expect(response.score).to eq 100
      expect(response.submitted_text).to eq opportunity.title
    ...
    it 'returns valid when errors found' do
      ...
      allow_any_instance_of(OpportunityQualityRetriever).to receive(:quality_check).with(Figaro.env.TG_HOSTNAME!, Figaro.env.TG_API_KEY!, opportunity.title).and_return(errors_result_json)
